...
from fastmri.models import UnetAttention
...
class UnetAttentionModule(MriModule):
    """
    ...
    def forward(self, image):
        output, attn_map = self.unet(image.unsqueeze(1))
        return output.squeeze(1), attn_map
        # return self.unet(image.unsqueeze(1)).squeeze(1)
  
    def training_step(self, batch, batch_idx):
        output, attn_map = self(batch.image)
    ...
    def test_step(self, batch, batch_idx):
        ...
        output, attn_map = self.forward(batch.image)
        mean = batch.mean.unsqueeze(1).unsqueeze(2)
        std = batch.std.unsqueeze(1).unsqueeze(2)
    ...
    def training_epoch_end(self, losses):
        super().training_epoch_end(losses)        
